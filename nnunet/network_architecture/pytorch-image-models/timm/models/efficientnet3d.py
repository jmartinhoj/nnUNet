...
from .efficientnet_blocks import SqueezeExcite, SqueezeExcite3d
...
class EfficientNetFeatures(nn.Module):
    ...
    def __init__(
        ...
        norm_act_layer = get_norm_act_layer(norm_layer, act_layer)
        se_layer = se_layer or SqueezeExcite3d
        self.drop_rate = drop_rate
...
def _gen_efficientnetv2_s_3d(
    ...
    model_kwargs = dict(
        ...
        stem_size=24,
        round_chs_fn=round_chs_fn,
        norm_layer=kwargs.pop('norm_layer', None) or partial(nn.BatchNorm3d, **resolve_bn_args(kwargs)),
        act_layer=resolve_act_layer(kwargs, 'silu'),
        **kwargs,
    )
    model = _create_effnet(variant, pretrained, **model_kwargs)
