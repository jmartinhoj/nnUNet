...
class nnUNetTrainer_efficient_profiler(nnUNetTrainerV2):
    ...
    def initialize_network(self, training=True):
        ...
        # self.network = Generic_FullyEffUNet(self.num_input_channels, self.base_num_features, self.num_classes,
        #                             len(self.net_num_pool_op_kernel_sizes),
        #                             self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
        #                             dropout_op_kwargs,
        #                             net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
        ...
        self.network = Fully_Efficient_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
                            ...
                            dropout_op_kwargs,
                            net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                            self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True, efficient_model="efficientnet-b1")




        inputs = torch.randn(2, 1,128,128,128).cuda()
